...
class Query_Frontend_Filters {
	...
	public static function parse_item_from_block( $block, $ql_query_meta, $query_builder, $query_args, $lang ) {
		...
		if ( 'kadence/query-filter-date' === $block['blockName'] && ! empty( $block['attrs']['uniqueID'] ) ) {
			...
			self::$return[ $attrs['uniqueID'] ] = self::build_generic_filter( $attrs, $source, $taxonomy, $exclude_values, $include_values, $show_children, $show_hierarchical, $show_result_count, $hide_when_empty_count, $post_type, $post_field, $placeholder, $block, $query_args, $hash, $meta_offset, $lang, $inherit );
		}
	}
	...
	public static function build_generic_filter( $attrs, $source, $taxonomy, $exclude_values, $include_values, $show_children, $show_hierarchical, $show_result_count, $hide_when_empty_count, $post_type, $post_field, $placeholder, $block, $query_args, $hash, $meta_offset, $lang, $inherit ) {
		...
		if( $show_result_count ) {
			self::update_result_counts( $options_array, $attrs, $source, $taxonomy, $exclude_values, $include_values, $show_children, $show_hierarchical, $show_result_count, $hide_when_empty_count, $post_type, $post_field, $query_args, $hash, $meta_offset, $lang, $inherit );
		} else if ( $hide_when_empty_count ) {
			self::update_result_counts_empty( $options_array, $attrs, $source, $taxonomy, $exclude_values, $include_values, $show_children, $show_hierarchical, $show_result_count, $hide_when_empty_count, $post_type, $post_field, $query_args, $hash, $meta_offset, $lang, $inherit );
		}
		...
		if ( 'taxonomy' == $source ) {
			...
			if ( 'wordpress' == $source && 'post_author' == $post_field ) {
				...
				if ( 'custom_field' === $post_field ) {
					...
					} else if ( 'kb_custom_input' === $custom_field ) {
					...
					if ( $is_multi_choice && isset( $field_object['choices'] ) && is_array( $field_object['choices'] ) ) {
						...
						$results = $results->getAll();
						if ( is_array( $results ) && $results ) {
							$options_array = array_map( function( $result ) use ( $attrs, $show_result_count, $meta_type, $actual_key ){
								if ( 'acf_meta' === $meta_type && function_exists( 'get_field_object' ) ) {
									...
									return self::option_format( $result->meta_value, $label, $result->count, $show_result_count, $result->meta_value );
								} else if ( 'mb_meta' === $meta_type && function_exists( 'rwmb_get_field_settings' ) ) {
	...
	public static function update_result_counts( &$options_array, $attrs, $source, $taxonomy, $exclude_values, $include_values, $show_children, $show_hierarchical, $show_result_count, $hide_when_empty_count, $post_type, $post_field, $query_args, $hash, $meta_offset, $lang, $inherit ) {
	...
	public static function update_result_counts_empty( &$options_array, $attrs, $source, $taxonomy, $exclude_values, $include_values, $show_children, $show_hierarchical, $show_result_count, $hide_when_empty_count, $post_type, $post_field, $query_args, $hash, $meta_offset, $lang, $inherit ) {
		$object_ids = self::get_object_ids( $query_args, $meta_offset, $inherit );
		$new_options_array = self::build_options_array( $attrs, $source, $taxonomy, $exclude_values, $include_values, $show_children, $show_hierarchical, $show_result_count, $hide_when_empty_count, $post_type, $post_field, $object_ids, $hash, $lang );
