 ...
 * @category class
 */
class Kadence_Blocks_Pro_Filter_Reset_Block extends Kadence_Blocks_Query_Children_Block {
	 ...
	 * @var string
	 */
	protected $block_name = 'filter-reset';
	...
	public function build_html( $attributes, $unique_id, $content, $block_instance ) {
		$class_id = $unique_id;
		$outer_classes = array( 'kadence-query-filter', 'wp-block-kadence-query-filter-reset', 'wp-block-kadence-query-filter-reset' . $unique_id );
		$wrapper_args = array(
			'class' => implode( ' ', $outer_classes ),
			'data-uniqueid' => $unique_id,
		);
		$wrapper_attributes = get_block_wrapper_attributes( $wrapper_args );
		$classes = array( 'kb-button', 'kt-button', 'button', 'kb-query-filter-reset-button', 'kb-btn' . $class_id );
	...
	public function build_css( $attributes, $css, $unique_id, $unique_style_id ) {
		...
		$css->set_selector( '.wp-block-kadence-query-filter-reset' . $unique_id . ' .kb-button' );
		if ( 'fixed' === $width_type ) {
		...
		// Icon.
		$css->set_selector( '.wp-block-kadence-query-filter-reset' . $unique_id . ' .kb-button .kb-svg-icon-wrap' );
		...
		// Icon Hover.
		$css->set_selector( '.wp-block-kadence-query-filter-reset' . $unique_id . ' .kb-button:hover .kb-svg-icon-wrap' );
		if ( ! empty( $attributes['iconColorHover'] ) ) {
			$css->add_property( 'color', $css->render_color( $attributes['iconColorHover'] ) );
		}
		// Hover.
		$css->set_selector( '.wp-block-kadence-query-filter-reset .kb-btn' . $class_id . '.kt-button:hover' );
		...
		if ( isset( $attributes['displayHoverShadow'] ) && true === $attributes['displayHoverShadow'] ) {
			if ( ( 'gradient' === $bg_type || 'gradient' === $bg_hover_type ) && isset( $attributes['shadowHover'][0]['inset'] ) && true === $attributes['shadowHover'][0]['inset'] ) {
				...
				$css->set_selector( '.wp-block-kadence-query-filter-reset' . $unique_id . ' .kb-button:hover::before' );
			}
		...
		if ( 'gradient' === $bg_type && 'normal' === $bg_hover_type && ! empty( $attributes['backgroundHover'] ) ) {
			$css->set_selector( '.wp-block-kadence-query-filter-reset' . $unique_id . ' .kb-button:hover::before' );
			$css->add_property( 'background', $css->render_color( $attributes['backgroundHover'] ) );
			$css->set_selector( '.wp-block-kadence-query-filter-reset' . $unique_id . ' .kb-button::before' );
		...
		if ( isset( $attributes['onlyIcon'][0] ) && '' !== $attributes['onlyIcon'][0] && true == $attributes['onlyIcon'][0] ) {
			$css->set_selector( '.wp-block-kadence-query-filter-reset' . $unique_id . ' .kb-button .kt-btn-inner-text' );
...
Kadence_Blocks_Pro_Filter_Reset_Block::get_instance();
