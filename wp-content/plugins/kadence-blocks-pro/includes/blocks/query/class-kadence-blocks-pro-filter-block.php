 ...
 * @category class
 */
class Kadence_Blocks_Pro_Filter_Block extends Kadence_Blocks_Query_Children_Block {
	/**
	...
	public function build_css( $attributes, $css, $unique_id, $unique_style_id ) {
		...
		$css->set_selector( 'body .wp-block-kadence-query.wp-block-kadence-query .wp-block-kadence-query-filter' . $unique_id . ' .kb-filter' );
		...
		$css->render_border_styles( $attributes );
	...
	public function build_html( $attributes, $unique_id, $content, $block_instance ) {
		...
		$outer_classes = array(
			...
			'wp-block-kadence-query-filter' . $unique_id,
		);
		$wrapper_args = array(
		...
		return sprintf(
			'<div %s><fieldset class="kadence-filter-wrap">%s<div class="filter-refresh-container">%s</div></fieldset></div>',
			$wrapper_attributes,
			$label_html,
			$filters
		);
...
Kadence_Blocks_Pro_Filter_Block::get_instance();
