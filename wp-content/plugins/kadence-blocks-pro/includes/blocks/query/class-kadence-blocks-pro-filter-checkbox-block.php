...
class Kadence_Blocks_Pro_Filter_Checkbox_Block extends Kadence_Blocks_Query_Children_Block {
	...
	public function build_html( $attributes, $unique_id, $content, $block_instance ) {
		...
		$outer_classes = array(
			...
			'wp-block-kadence-query-filter-checkbox' . $unique_id,
		);
		...
		return sprintf(
			'<div %s><fieldset class="kadence-filter-wrap">%s<div class="checkbox-options filter-refresh-container">%s</div></fieldset></div>',
			$wrapper_attributes,
			$label_html,
			$filters
		);
