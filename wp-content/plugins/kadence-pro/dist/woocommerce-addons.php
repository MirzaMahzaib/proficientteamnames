...
class Woocommerce_Addons {
	...
	public function add_mini_cart_notice() {
		if ( kadence()->option( 'cart_pop_show_free_shipping' ) && function_exists( 'WC' ) && WC()->cart ) {
			$min_amount = kadence()->option( 'cart_pop_free_shipping_price' );
			$current = ( WC()->cart->get_tax_price_display_mode() == 'incl' ? WC()->cart->get_cart_contents_total() + WC()->cart->get_cart_contents_tax() : WC()->cart->get_cart_contents_total() );
	...
	public static function add_to_cart() {
		...
		if ( $product && ( 'variation' === $product->get_type() || 'subscription_variation' === $product->get_type() ) ) {
	...
	public function add_option_defaults( $defaults ) {
		$woo_addons = array(
			...
			'cart_pop_free_shipping_calc_tax' => false,
